...
def formulaire_textgrp():
    ...
    rtype: list
    """
    code_textgrp = input("Quel est le code du corpus (<textgroup>) ? : ")
...
def formulaire_work():
    ...
    """
    code_textgrp = input("Quel est le code du corpus (<textgroup>) ? : ")
    code_work = input("Quel est le code de l'oeuvre (<work>) ? : ")
    ...
    lang_edition = input("Quelle est la langue utilisée pour nommer cette édition ? : ")
    while len(lang_edition) != 3 :
        print("Erreur sur le format de la langue du nom d'edition : veuillez entrer un code à 3 lettres")
        lang_edition = input("Quelle est la langue utilisée pour nommer cette edition ? : ")

    label_edition = input("Quel est le nom de la présente edition de l'oeuvre (balise <label>) ? : ")
    lang_label_ed = input("Quelle est la langue utilisée pour le contenu du label ? : ")
    while len(lang_label_ed) != 3 :
        print("Erreur sur le format de la langue du label : veuillez entrer un code à 3 lettres")
        lang_label_ed = input("Quelle est la langue utilisée pour le contenu du label ? : ")
    ...
    resultat = [chemin, code_textgrp, code_work, lang_titre_work, titre_work, label_edition, lang_label_ed, code_edition, desc_ed, lang_desc_ed, lang_work, lang_edition]
    return resultat
...
def valider_work(liste_resultat):
    ...
    print("L'élément <edition> aura pour attribut : @xml:lang= \"" + liste_resultat[11] + "\"")
    ...
    print("L'élément <label> contiendra le texte suivant : " + liste_resultat[5])
    print("L'élément <label> aura pour attribut : @xml:lang=\"" + liste_resultat[6] + "\"")
...
def create_cts_work (liste_information):
    ...
    '''
    import lxml.etree as ET
    urn_group = 'urn:cts:'+ CODE_NAMESPACE + ":" + liste_information[1]
    urn_work = urn_group + '.' + liste_information[2]
    urn_edition = urn_work + "." + liste_information[7]
    ...
    edition.attrib['workUrn'] = urn_work
    edition.attrib['urn'] = urn_edition
    attr_lang_edition = edition.attrib
    attr_lang_edition['{http://www.w3.org/XML/1998/namespace}lang'] = liste_information[11]

    label = ET.SubElement(edition, "label")
    attr_lang_label = label.attrib
    attr_lang_label['{http://www.w3.org/XML/1998/namespace}lang'] = liste_information[6]
    label.text = liste_information[5]
    ...
    description.text = liste_information[8]
